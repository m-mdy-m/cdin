...
local function splice(t, at, remove, insert)
  ...
  local old_len = #t
  if offset < 0 then
    for i = at - offset, old_len - offset do t[i + offset] = t[i] end
  elseif offset > 0 then
    for i = old_len, at, -1 do t[i + offset] = t[i] end
  end
  for i, item in ipairs(insert) do t[at + i - 1] = item end
end
...
local function push_undo(stack, time, type, ...)
  stack[stack.idx] = { type = type, time = time, ... }
  stack[stack.idx - config.max_undos] = nil
...
function Doc:new(filename)
  self:reset()
  if filename then self:load(filename) end
end
...
function Doc:get_change_id() return self.undo_stack.idx end
function Doc:is_dirty()     return self.clean_change_id ~= self:get_change_id() end
function Doc:clean()        self.clean_change_id = self:get_change_id() end
...
function Doc:get_selection(sort)
  ...
  if sort then return sort_positions(a.line, a.col, b.line, b.col) end
...
local function position_offset_linecol(self, line, col, dl, dc)
  return self:sanitize_position(line + dl, col + dc)
end

function Doc:position_offset(line, col, ...)
  if type(...) ~= "number" then
    return (select(1, ...))(self, line, col, select(2, ...))
...
function Doc:get_text(line1, col1, line2, col2)
  ...
  if line1 == line2 then return self.lines[line1]:sub(col1, col2 - 1) end
  local res = { self.lines[line1]:sub(col1) }
  for i = line1 + 1, line2 - 1 do res[#res + 1] = self.lines[i] end
  res[#res + 1] = self.lines[line2]:sub(1, col2 - 1)
...
function Doc:raw_insert(line, col, text, undo_stack, time)
  ...
  for i = 1, #lines - 1 do lines[i] = lines[i] .. "\n" end
...
function Doc:undo() pop_undo(self, self.undo_stack, self.redo_stack) end
function Doc:redo() pop_undo(self, self.redo_stack, self.undo_stack) end

function Doc:text_input(text)
  if self:has_selection() then self:delete_to() end
