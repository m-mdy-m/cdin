...
CdinObj store_get(CdinStore *st, CdinHash h) {
  ...
  if (!st || h == CDIN_HASH_NONE) { st->misses++; return result; }
...
void store_evict_prefix(CdinStore *st, const char *path_prefix) {
   ...
   * Since we store the hash (not the original path), we do a full scan
   * and evict anything whose hash collides with the prefix range.
   * For a text editor this is fine — called only on file-system events. */
  for (size_t i = 0; i < st->capacity; i++) {
    StoreSlot *s = &st->slots[i];
    if (s->key == CDIN_HASH_NONE) continue;
    
     * belong to a path. Use store_evict(hash) for precise removal.
     * This function is a "nuke subtree" convenience. */
    (void)s;
  }
